...
if (typeof (AjaxQuickDelete) == 'undefined' && wgNamespaceNumber >= 0) {
   ...
   var AjaxQuickDelete = {
      ...
      /**
      ** Set up the AjaxQuickDelete object and add the toolbox link.  Called via $j(document).ready() during page loading.
      **/
      install : function () {
          ...
          if (this.userRights == 'sysop' && wgNamespaceNumber == 6) {
            if ($j('#AjaxDupeProcess').length) $j('#AjaxDupeProcess').append('<a href="javascript:AjaxQuickDelete.processDupes();">Process Duplicates</a>').show();
          }
      ...
      **/
      moveFile: function () {
          ...
          if ($j('#globalusage').length || !$j('#mw-imagepage-nolinkstoimage').length ) this.inUse=true;
          ...
          this.prompt ([{
            ...
            if (this.inUse || this.userRights == 'filemover') $j('#AjaxQuestion2').attr('disabled', true);
      },
      ...
      **/
      ...
      processDupes : function (){
          ...
          this.destination = $j('#AjaxDupeDestination').html();
          this.nextTask();
          ...
          for (var id in pages) {  // there should be only one, but we don't know its ID
              ...
              if (wgNamespaceNumber != 6) {
                ...
                for (var i = info.length-1; i >= 0; i--) {  // iterate in reverse order
                    ...
                    if(wgUserName == info[i].user) continue;
      ...
      **/
      prompt: function (questions, title, width) {
          ...
          dlgButtons[this.i18n.submitButtonLabel] = function () {
              $j.each(questions, function(i, v) {
                  response = $j('#AjaxQuestion' + i).val();
                  if (v.type == 'checkbox') response = $j('#AjaxQuestion' + i).attr('checked');
                  if (v.cleanUp) {
          ...
          $j.each(questions, function(i, v) {
              ...
              $j('#AjaxQuestion' + i).val(v.prefill);
              if (v.type == 'checkbox') $j('#AjaxQuestion' + i).attr('checked', v.prefill).attr('style', 'margin-left: 5px');
              $j('#AjaxQuestion' + i).keyup();
          });
          $j('#AjaxQuestion0').focus();
      },
      ...
      **/
      compareDetails: function () {
          ...
          $j('#AjaxDupeContainer').append('<div><img src="' + d.thumburl + '" ></div><div><img src="' + f.thumburl + '"></div><br>')
      ...
      **/
      ...
      cleanFileName : function (uncleanName){
          ...
          currentExtension = wgPageName.toLowerCase().replace(/.*?\./, '').replace('jpeg', 'jpg');
      ...
      **/
      showProgress : function (message) {
          if ($j('#feedbackContainer').length) {
              $j('#feedbackContainer').html(message);
          } else {
              document.body.style.cursor = 'wait';

              this.progressDialog = $j('<div></div>')
      ...
      **/
      fail : function ( err ) {
          ...
          $j('.ui-dialog-content').height('auto');
          $j('.ui-dialog').addClass('ajaxDeleteError');
 ...
 $j(document).ready(function() { AjaxQuickDelete.install(); });
